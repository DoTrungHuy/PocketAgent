...
[直接下载 AgentPad v0.2.1-alpha.1 APK](https://github.com/DoTrungHuy/AgentPad/releases/download/v0.2.1-alpha.1/AgentPad-v0.2.1-alpha.1.apk)

[查看发布说明、SHA256、签名证书指纹和 SBOM](https://github.com/DoTrungHuy/AgentPad/releases/tag/v0.2.1-alpha.1)

Android 会显示标准安装确认。AgentPad 不要求安装 Termux、Google Play、Google 登录或 Google Services。升级包继续使用 v0.2.0 的正式签名，可覆盖安装并保留本地数据。

> `v0.2.1-alpha.1` 是预发布测试版本。长期下载入口只指向正式签名 APK；名称含短 Git SHA 的 Debug 产物仅供开发排错。

`main` 分支正在开发 `v0.2.2-alpha.1`：重点改成更接近 Codex / Zed 的简洁工作台，保留聊天、任务审批和模型设置三条主线。正式 APK 发布前，下载入口仍指向上一个已签名预览版。

## v0.2.2 开发重点

- 简洁工作台：左侧会话、中央聊天、固定底部输入区，大屏不再堆叠复杂右栏。
- 主要页面收敛为 Chat、Tasks、Settings；计划和审批直接显示在任务卡里。
