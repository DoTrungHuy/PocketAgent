...
1. 从可信来源安装 Termux。
2. 把完整 AgentPad 源码目录传到平板。
3. 在目录内运行 `bash install.sh`。
4. 确认 AgentPad 安装过程没有新增或调用 PRoot、Ubuntu、Node.js 或 Google 服务；设备原本安装这些软件不算失败，但必须记录。
5. 记录安装耗时、下载量和最终占用空间。
...
1. 至少配置一个中国大陆模型服务。
2. 运行 `agentpad doctor --test-api`。
3. 使用 `agentpad chat` 完成连续三轮对话。
4. 让 Agent 列出工作目录。
...
6. 运行 `agentpad stop`，确认端口关闭。

## 故障模拟

- 断网后运行 `agentpad doctor`。
- 使用错误 API Key 运行 `agentpad doctor --test-api`。
- 填写不存在的模型名称。
...
把生成的 `AgentPadReport-日期时间.txt` 文件连同以下内容发给开发者：
