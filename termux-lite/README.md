...
PocketAgent 是一个从安卓平板实际能力出发设计的轻量 AI Agent，而不是某个桌面 Agent 的安卓搬运版。
...
完整 Agent 框架在桌面和服务器上功能丰富，但放到安卓平板后经常需要 Linux 容器、Node.js、原生模块或大量 Python 依赖。PocketAgent 首版只保留平板真正需要的部分：
...
OpenClaw、nanobot 等框架未来可以作为高级后端，但不是 PocketAgent 的运行前提。
...
```text
pocketagent configure          配置模型、工作目录和工具权限
pocketagent doctor             检查环境与网络
pocketagent doctor --test-api  真实调用一次模型 API
pocketagent report             生成不调用模型的脱敏实机报告
pocketagent report --test-api  报告中加入一次真实模型测试
pocketagent chat               Termux 终端对话
pocketagent start              后台启动 Web UI
pocketagent stop               停止 Web UI
pocketagent restart            重启 Web UI
pocketagent status             查看状态
pocketagent logs               查看日志
pocketagent update             更新程序
...
```text
~/.pocketagent/config.json       非敏感配置
~/.pocketagent/secrets.env       API Key，权限 600
~/.pocketagent/sessions/         本地会话
~/.pocketagent/logs/             运行日志
~/PocketAgentWorkspace/          Agent 默认工作目录
```

API Key 不会写入 `config.json`。程序在错误、网页响应和常见日志中会尝试脱敏，但仍不应把整个 `~/.pocketagent` 目录上传或分享。
...
PocketAgent 的轻量核心不需要 npm、PyPI 或 Ubuntu 软件源，因此避开了三类常见安装故障。

- Termux 软件包仍通过用户当前配置的软件源安装。
- 模型请求直接访问用户选择的中国大陆服务商。
- GitHub 只在下载或更新 PocketAgent 时需要，不影响日常运行。
...
报告默认生成在 Termux 主目录，文件名类似 `PocketAgentReport-20260610-103000.txt`。它不会采集 API Key、设备序列号或 Android ID。发送前仍建议自己打开检查一次。
...
- `pocketagent update` 在公开发布前只支持重新安装本地源码。
...
```bash
shellcheck bootstrap.sh install.sh pocketagent
```
...
PocketAgent 不复制这些项目的内核；它们用于比较功能边界和后续可选后端。
